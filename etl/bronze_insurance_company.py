# ---------------------------------------------------------------------
# bronze_insurance_company.py
# ---------------------------------------------------------------------
# Create MinIO Parquet version of Insurance Company dimension from CDWWork.
#  - pull from Dim.InsuranceCompany
#  - save to med-z1/bronze/cdwwork/insurance_company_dim
#  - as insurance_company_dim_raw.parquet
# ---------------------------------------------------------------------
...
def extract_insurance_company_bronze():
    """Extract insurance company dimension data from CDWWork to Bronze layer in MinIO."""
    logger.info("Starting Bronze insurance company dimension extraction")
    ...
    logger.info(f"Extracted {len(df)} insurance companies from CDWWork")
    ...
    object_key = build_bronze_path(
        ...
        domain="insurance_company_dim",
        filename="insurance_company_dim_raw.parquet"
    )
    ...
    logger.info(
        f"Bronze extraction complete: {len(df)} insurance companies written to "
...
if __name__ == "__main__":
    ...
    extract_insurance_company_bronze()
